...
.then(function({data: authorCommitsJson}) {
	...
	for (var i=0; i<25; i++) {
	  if(authorCommitsJson[i].sha.match(re)) {
	       authorCommitsJson[i].isNum = true; 
	    }
	  else {
	        authorCommitsJson[i].isNum = false;
	    }
		data.push(authorCommitsJson[i]);
    }	
//Render the results in HTML:  		
	res.render('commit', {title: ' Code Challenge 2', repo: repoParsed, data: data, re: re}); 
